...
This class creates a new `CampaignMember` for each Closed-Won Opportunity belonging to a Campaign that has a Contact role defined. It also creates a relationship between the referring Contact and the new Campaign member. Note that because the class is external, it needs to declare the method global, and needs to use the npsp prefix when calling classes inside the package.
